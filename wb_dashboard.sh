...
set_language() {
    ...
    case "$CURRENT_LANG" in
        ru)
            ...
            L_WIZ_TITLE="НАСТРОЙКА СЕРВЕРА И АККАУНТА"
        ...
        pt)
            ...
            L_WIZ_TITLE="CONFIGURACAO DO SERVIDOR E CONTA"
        ...
        *)
            ...
            L_WIZ_TITLE="SERVER & ACCOUNT CONFIGURATION"
    ...
    if [[ -f "$CONFIG_FILE" ]]; then
        while IFS='=' read -r key val || [[ -n "$key" ]]; do
            ...
            case "$key" in
                ...
                BOT_RANK) [[ -n "$val" ]] && BOT_RANK="$val" ;;
                BOT_CLAN) [[ -n "$val" ]] && BOT_CLAN="$val" ;;
                BOT_MONEY) [[ -n "$val" ]] && BOT_MONEY="$val" ;;
                BOT_CROWNS) [[ -n "$val" ]] && BOT_CROWNS="$val" ;;
                ROOM_NAME) [[ -n "$val" ]] && ROOM_NAME="$val" ;;
                ROOM_MODE) [[ -n "$val" ]] && ROOM_MODE="$val" ;;
                ROOM_MASTER) [[ -n "$val" ]] && ROOM_MASTER="$val" ;;
                ROOM_PLAYERS) [[ -n "$val" ]] && ROOM_PLAYERS="$val" ;;
                BOT_CLASS_ID) [[ -n "$val" ]] && BOT_CLASS_ID="$val" ;;
                STATUS_READY) [[ -n "$val" ]] && STATUS_READY="$val" ;;
                STATUS_FOLLOW) [[ -n "$val" ]] && STATUS_FOLLOW="$val" ;;
            esac
        done < "$CONFIG_FILE"
    fi
}
...
run_setup_wizard() {
    ...
    echo -ne "  ${C_YELLOW}$L_WIZ_TOKEN ${C_WHITE}[${BOT_TOKEN}]: ${RESET}"
    read -r in_token
...
menu_conectar_bot() {
    ...
    echo -e "${C_YELLOW}Comandos disponiveis no prompt CMD# do bot:${RESET}"
    echo -e "  ${C_CYAN}friends              ${C_WHITE}- Lista amigos no servidor"
    echo -e "  ${C_CYAN}follow <nick>        ${C_WHITE}- Segue jogador para a sala"
    echo -e "  ${C_CYAN}invite <nick>        ${C_WHITE}- Convida amigo para a sala"
    echo -e "  ${C_CYAN}ready [classe]       ${C_WHITE}- Fica pronto / altera classe"
    echo -e "  ${C_CYAN}unready              ${C_WHITE}- Cancela prontidao"
    echo -e "  ${C_CYAN}open <missao>        ${C_WHITE}- Cria sala PvE ou PvP"
    echo -e "  ${C_CYAN}start                ${C_WHITE}- Inicia a partida na sala"
    echo -e "  ${C_CYAN}leave                ${C_WHITE}- Sai da sala atual"
    echo -e "  ${C_CYAN}master <nick>        ${C_WHITE}- Passa lideranca da sala"
    echo -e "  ${C_CYAN}say <msg>            ${C_WHITE}- Chat da sala"
    echo -e "  ${C_CYAN}whisper <nick> <msg> ${C_WHITE}- Sussurro privado"
    echo -e "  ${C_RED}quit                 ${C_WHITE}- Desconecta e volta ao menu"
    echo -e "$BARRA"
    echo ""
...
menu_classe() {
    ...
    case "$c_opt" in
        1|2|3|4)
            ...
            log_event "Classe alterada para [$(get_class_name)]"
            save_bot_config
            ;;
        0) return;;
