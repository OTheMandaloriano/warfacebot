...
В этот форк добавлен скрипт `wb_dashboard.sh`: эргономичная консольная панель управления для быстрого администрирования игровых сессий:

```text
  ========================================================================
    WARFACEBOT • ПАНЕЛЬ УПРАВЛЕНИЯ | Levak
  ========================================================================
   БОТ: Shadow_104     СЕРВЕР: 127.0.0.1:5222    СТАТУС: ОНЛАЙН [OK]
  ========================================================================
  ПРОФИЛЬ БОТА            ИГРОВАЯ КОМНАТА           СИСТЕМА И СЕТЬ
  Ник:      Shadow_104    Комната:  Лобби_PvE       Пинг:      12мс
  Ранг:     1             Миссия:   Обычная         Аптайм:    00:15:32
  Клан:     Без клана     Глава:    Shadow_104      Память:    34.5 МБ
  Варбаксы: 50000         Игроки:   1/5             Потоки:    5 Active
  Короны:   500
  ========================================================================
  КЛАСС: [Штурмовик]  ГОТОВ: НЕТ  АВТО-СЛЕДОВАНИЕ: ДА
  ========================================================================

  [01] • ПОДКЛЮЧИТЬСЯ К ИГРЕ (СЕССИЯ)     [08] • СЛЕДОВАТЬ ЗА ИГРОКОМ
  [02] • СМЕНИТЬ КЛАСС                    [09] • ПЕРЕДАТЬ ЛИДЕРА
  [03] • ГОТОВ / НЕ ГОТОВ                 [10] • ПРИГЛАСИТЬ ДРУГА
  [04] • СОЗДАТЬ PVE КОМНАТУ              [11] • ДОБАВИТЬ В ДРУЗЬЯ
  [05] • СОЗДАТЬ PVP КОМНАТУ              [12] • СПИСОК ДРУЗЕЙ
  [06] • НАЧАТЬ МАТЧ                      [13] • ИНФОРМАЦИЯ О БОТЕ
  [07] • ВЫЙТИ ИЗ КОМНАТЫ                 [14] • НАСТРОЙКА СЕРВЕРА/АККАУНТА
                                          [15] • СМЕНИТЬ ЯЗЫК
                                          [00] • ОТКЛЮЧИТЬСЯ И ВЫЙТИ <<<

  ========================================================================
  [*] ПОСЛЕДНИЕ СОБЫТИЯ И УВЕДОМЛЕНИЯ:
    [18:30:10] Успешное подключение к XMPP серверу (127.0.0.1:5222).
    [18:30:30] Бот готов к работе под ником: Shadow_104.
...
### Преимущества панели
- **Мастер начальной настройки (Setup Wizard)**: При первом запуске скрипт запрашивает язык (`Русский`, `English`, `Português`) и помогает пошагово настроить подключение: адрес сервера, порт, версию Warface и данные аккаунта (с автоматической генерацией позывного, например `Shadow_421`).
- **Авто-вход и постоянная конфигурация**: Настройки безопасно сохраняются в локальный файл `.wb_config` и генерируют `cfg/server/custom.cfg` (оба исключены в `.gitignore`). При следующих запусках бот подключается мгновенно без повторного ввода данных.
- **Реальное подключение к игре [01]**: Запускает исполняемый файл `wb.exe` с введенным токеном и UID для входа в лобби сервера.
- **Помощь по командам шепотом в игре**: При отправке любого сообщения в ЛС бота (`/w <НикБота> help`) он сразу высылает в ответ список доступных команд:
  `[WarfaceBot] Команды: ready [класс] | unready | switch | start | leave | master | stay | invite`
- **Управление друзьями и авто-прием [11]**: Бот автоматически подтверждает входящие заявки в друзья и позволяет отправлять приглашения в друзья прямо из терминала.
- **Быстрая смена языка [15]**: Переключение между русским, английским и португальским в любой момент.
- **Гибридный ввод**: Принимает как номера опций меню (`01` - `15`), так и прямые команды бота (`open`, `say`, `quit`, `stats`).
